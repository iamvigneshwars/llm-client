...
const App: React.FC = () => {
  ...
  const checkConnection = async () => {
    try {
      const response = await fetch('http://172.23.162.4:5000/health', {
        method: 'GET',
      });
  ...
  const saveHistory = (question: string, response: string, metadata: string) => {
    try {
      ...
      const historyFile = localStorage.getItem('chatbot_logs');
      let history = historyFile ? JSON.parse(historyFile) : [];
      
      history.push(entry);
      localStorage.setItem('chatbot_logs', JSON.stringify(history, null, 2));
    } catch (error) {
  ...
  const handleSend = async (event?: React.KeyboardEvent) => {
    ...
    try {
      const response = await fetch('http://172.23.162.4:5000/ask', {
        method: 'POST',
        headers: {
